...
const ServiceDetails: React.FC<ServiceDetailsProps> = ({ booking, onUpdate }) => {
  ...
  const toggleDriverAvailability = () => {
    ...
    const newStatus = booking.driver.availability === 'Available' ? 'On Another Tour' : 'Available';
    onUpdate({
      ...booking,
      driver: {
        ...booking.driver,
      ...
      <div className="space-y-4 sm:space-y-6 min-h-[400px]">
        ...
        {activeTab === 'info' && (
          <div className="space-y-4 sm:space-y-6">
            ...
            {/* Driver Expert Card with Availability */}
            {booking.driver && (
              <div className="bg-white rounded-[24px] sm:rounded-[32px] p-6 sm:p-8 border border-slate-100 shadow-sm overflow-hidden relative">
                <div className="flex justify-between items-start mb-4 sm:mb-6">
                  <h3 className="text-[8px] sm:text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Local Expert</h3>
                  <div className="flex items-center gap-2">
                    <span className={`w-2 h-2 rounded-full ${booking.driver.availability === 'Available' ? 'bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]' : 'bg-amber-500 shadow-[0_0_8px_rgba(245,158,11,0.5)]'}`} />
                    <span className={`text-[8px] sm:text-[9px] font-bold uppercase tracking-widest ${booking.driver.availability === 'Available' ? 'text-emerald-600' : 'text-amber-600'}`}>
                      {booking.driver.availability || 'Available'}
                    </span>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <a 
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="flex-1 py-3.5 sm:py-4 bg-emerald-500 text-white rounded-[16px] sm:rounded-[20px] flex items-center justify-center gap-2 sm:gap-3 hover:bg-emerald-600 shadow-lg shadow-emerald-500/20 active:scale-[0.98] transition-all"
                  >
                  ...
                  <button 
                    ...
                    className="flex-1 py-3.5 sm:py-4 bg-slate-900 text-white rounded-[16px] sm:rounded-[20px] flex items-center justify-center gap-2 sm:gap-3 hover:bg-slate-800 shadow-lg active:scale-[0.98] transition-all"
                  >
